...
bool SaveContentToFile(const std::tstring& sFilePath, const unsigned char *pFileContent, unsigned int nFilecontentSize, unsigned int nSkippedFrameCount /*= 0*/)
{
    int ret = -1;
    int nRet;
    FILE *pFile = NULL;
    const unsigned char *pCurPos = pFileContent;
    const unsigned char *pEndPos = pCurPos + nFilecontentSize;
    ...
    if (nSkippedFrameCount != 0) {
        ret = fseek(pFile, nFilecontentSize * nSkippedFrameCount, SEEK_SET);
        if (ret != 0) {
            ret = 0;
            fprintf(stderr, "*** ERROR: can't seek on this file: %s\n", sFilePath.c_str());
        }
