...
static void registry_add_object (void *data, struct wl_registry *registry, uint32_t name, const char *interface, uint32_t version) {
    if (!strcmp(interface,"wl_compositor")) {
        compositor = (wl_compositor*)wl_registry_bind (registry, name, &wl_compositor_interface, 0);
    }
    else if (!strcmp(interface,"wl_shell")) {
        shell = (wl_shell*)wl_registry_bind (registry, name, &wl_shell_interface, 0);
    }
}
...
static void create_window (struct window *window, int32_t width, int32_t height) {
    ...
    wl_shell_surface_set_title(window->shell_surface, "open_gl_sample");
