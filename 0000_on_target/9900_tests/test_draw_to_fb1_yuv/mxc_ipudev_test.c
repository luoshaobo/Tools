...
void setConfiguration(ipu_test_handle_t *test_handle)
{
    struct ipu_task *t = &test_handle->task;
    ...
    t->input.width = SCREEN_WIDTH;
    t->input.height = SCREEN_HEIGHT;
    t->input.format = v4l2_fourcc('B', 'G', 'R', 'A');
    t->input.crop.pos.x = 0;
    t->input.crop.pos.y = 0;
    t->input.crop.w = SCREEN_WIDTH;
    t->input.crop.h = SCREEN_HEIGHT;
    ...
    t->output.width = SCREEN_WIDTH;
    t->output.height = SCREEN_HEIGHT;
    t->output.format = v4l2_fourcc('U', 'Y', 'V', 'Y');
    t->output.rotate = 0;
    t->output.crop.pos.x = 0;
    t->output.crop.pos.y = 0;
    t->output.crop.w = SCREEN_WIDTH;
    t->output.crop.h = SCREEN_HEIGHT;
    test_handle->show_to_fb = 1;

    //memcpy(test_handle->outfile, "ipu0-1st-ovfb", strlen("ipu0-1st-ovfb"));
    memcpy(test_handle->outfile, "ipu0-1st-ovfb", strlen("ipu0-1st-ovfb"));
}
...
static const void *g_buf_argb = NULL;
static void *g_buf_yuv = NULL;

extern void colorSpaceConvert_DoTask();
void colorSpaceConvert(const void *buf_argb, void *buf_yuv, const struct Rect *rect)
{
    ...
    setConfiguration(&test_handle);

    t = &test_handle.task;

    t->input.crop.pos.x = rect->x;
    t->input.crop.pos.y = rect->y;
    t->input.crop.w = rect->w;
    t->input.crop.h = rect->h;

    t->output.crop.pos.x = rect->x;
    t->output.crop.pos.y = rect->y;
    t->output.crop.w = rect->w;
    t->output.crop.h = rect->h;

    colorSpaceConvert_DoTask();
}

//int main_orig(int argc, char *argv[])
void colorSpaceConvert_DoTask()
{
	struct ipu_task *t = &test_handle.task;
...
//		}
	...
	while((done_cnt < test_handle.fcount) && (ctrl_c_rev == 0)) {
		...
		if (t->input.deinterlace.enable &&
            ...
            memcpy(vdibuf, g_buf_argb, isize);
            ret = isize;
		} else
            //ret = fread(inbuf, 1, isize, file_in);
            memcpy(inbuf, g_buf_argb, isize);
            ret = isize;
		if (ret < isize) {
			ret = -1;
			printf("Can not read enough data from input file\n");
