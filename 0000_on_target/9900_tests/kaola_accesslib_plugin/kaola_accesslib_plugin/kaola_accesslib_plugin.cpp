...
namespace KaolaApiImpl {

using namespace KaolaApi;

OpSerialNumber g_opSerialNumber(OpSerialNumber_INAVLID);

class KaolaCommon : public ICommon
{
public:
    KaolaCommon(Callback *pCallback) : m_pCallback(pCallback), m_domainAvailability(false)
    {
        LOG_GEN();
    }
...
class KaolaPlayer : public IPlayer
{
public:
    KaolaPlayer(Callback *pCallback) : m_pCallback(pCallback), m_cycleMode(CycleMode_OrderCycle), m_playStatus(PlayStatus_Stopped)
    {
        LOG_GEN();
    }
...
class KaolaInterfaceImpl : public Interface
{
public:
    KaolaInterfaceImpl(Callback *pCallback) : m_kaolaCommon(pCallback), m_kaolaPlayer(pCallback)
    {
        LOG_GEN();
    }

    virtual ~KaolaInterfaceImpl()
    {
        LOG_GEN();
    }
    ...
    virtual bool initialize()
    {
        ...
        m_kaolaCommon.initialize();
        m_kaolaPlayer.initialize();
        return true;
    }
    ...
    virtual bool deinitialize()
    {
        ...
        m_kaolaCommon.deinitialize();
        m_kaolaPlayer.deinitialize();
        return true;
    }
    
    virtual ICommon &getCommonInterface()
    {
        return m_kaolaCommon;
    }
    
    virtual IPlayer &getPlayerInterface()
    {
        return m_kaolaPlayer;
    }
    
public:
    KaolaCommon m_kaolaCommon;
    KaolaPlayer m_kaolaPlayer;
};

} // namespace KaolaApiImpl {

extern "C" {
KaolaApi::Interface *getKaolaInterface(KaolaApi::Callback *pCallback)  // NOTE: this function will be exported in the DLL, and be loaded dynamically by HMI application
{
    static KaolaApiImpl::KaolaInterfaceImpl aKaolaInterfaceImpl(pCallback);
    bool bSuc = true;
    KaolaApi::Interface *pInterface = &aKaolaInterfaceImpl;
    ...
    if (bSuc) {
        if (pCallback == NULL) {
            ...
            //
            LOG_PRINTF("*** Error: getKaolaInterface(): pCallback == NULL\n");
        }
    }

    if (bSuc) {
        if (pCallback->getApiVersion() != KaolaApi::currentApiVersion) {
            pInterface = NULL;
            bSuc = false;
            ...
            //
            LOG_PRINTF("*** Error: getKaolaInterface(): version not matched\n");
        }
    }
