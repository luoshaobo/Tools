...
{
    if (pName == NULL) {
        char pTemp[] = "SharedMem_XXXXXXXXXX";
...
std::string SMF_PlatformSharedMemImpl::GetSharedMemName() const
{
    ...
    sFilePath = TK_Tools::FormatStr("%s/%s_SMF_PlatformSharedMemImpl_shm_%s", SMF_TEMP_DIR_ROOT, SMF_GetDomainStr(m_bSharedInProcesses).c_str(), m_sName.c_str());
    TK_Tools::GetFileINode(sFilePath, sResultTmp);
    sResult = TK_Tools::FormatStr("%s_shm_%s", SMF_GetDomainStr(m_bSharedInProcesses).c_str(), sResultTmp.c_str());
