...
SMF_MailBoxFactory::SMF_MailBoxFactory()
    ...
    , m_oLock("MailBoxFactory_Mutex", true)
    , m_oSharedMem(sizeof(SharedMailBoxSetInfo), "MailBoxFactory_SharedMem", true)
    , m_pSharedMem(NULL)
...
SMF_ErrorCode SMF_BaseMailBox::Bind(MailBoxId nMailBoxId)
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        ...
        m_pQNotFullEvent = new SMF_PlatformEvent(FALSE, FALSE, m_sQNotFullEventName.c_str(), true);

        TK_Tools::FormatStr(m_sQNotEmptyEventName, "BaseMailBox_%u_QNotEmptyEvent", m_nMailBoxId);
        m_pQNotEmptyEvent = new SMF_PlatformEvent(FALSE, FALSE, m_sQNotEmptyEventName.c_str(), true);
    }
