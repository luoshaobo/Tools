...
SMF_RpcTunnelFactory::SMF_RpcTunnelFactory()
    ...
    , m_oLock("RpcTunnelFactory_Mutex", true)
    , m_oSharedMem(sizeof(SharedRpcTunnelSetInfo), "RpcTunnelFactory_SharesMem", true)
    , m_pSharedMem(NULL)
...
SMF_ErrorCode SMF_RpcTunnel::Bind(RpcTunnelId nRpcTunnelId)
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        ...
        m_pLock = new SMF_PlatformLock(m_sLockName.c_str(), true);

        TK_Tools::FormatStr(m_sNoCallNorResultEventName, "RpcTunnel_%u_NoCallNorResultEvent", m_nRpcTunnelId);
        m_pNoCallNorResultEvent = new SMF_PlatformEvent(FALSE, FALSE, m_sNoCallNorResultEventName.c_str(), true);

        TK_Tools::FormatStr(m_sCallPutInEventName, "RpcTunnel_%u_CallPutInEvent", m_nRpcTunnelId);
        m_pCallPutInEvent = new SMF_PlatformEvent(FALSE, FALSE, m_sCallPutInEventName.c_str(), true);

        TK_Tools::FormatStr(m_sResultPutInEventName, "RpcTunnel_%u_ResultPutInEvent", m_nRpcTunnelId);
        m_pResultPutInEvent = new SMF_PlatformEvent(FALSE, FALSE, m_sResultPutInEventName.c_str(), true);

        TK_Tools::FormatStr(m_sCallProcEndEventName, "RpcTunnel_%u_CallProcEndEvent", m_nRpcTunnelId);
        m_pCallProcEndEvent = new SMF_PlatformEvent(FALSE, FALSE, m_sCallProcEndEventName.c_str(), true);
    }
