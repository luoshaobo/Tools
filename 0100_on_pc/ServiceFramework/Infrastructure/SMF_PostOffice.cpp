...
SMF_BasePostOffice::SMF_BasePostOffice()
    ...
    , SMF_ThreadHandler()
    , m_oLock("BasePostOffice_Lock", true)
    , m_oSubscriberMap()
    , m_oPublisherMap()
    , m_oSharedMem(sizeof(PubSubInfo), "BasePostOffice_SharedMem", true)
    , m_pSharedMem(NULL)
    , m_pPubSubInfo(NULL)
    , m_pPubInfo(NULL)
    , m_pSubInfo(NULL)
    ...
    if (nErrorCode == SMF_ERR_OK) {
	    ...
	    for (;;)
	    {
            ...
            if (!sock.recvfrom(pBuf, nSize)) {
                SMF_LOG_ERR("*** ERROR: [%s()] sock.recvfrom() is failed\n", __FUNCTION__);
			    continue;
		    }
#else
            ...
            if (nErrorCode != SMF_ERR_OK) {
                SMF_LOG_ERR("*** ERROR: [%s()] oMailBox.GetMail() is failed, exits thread ...\n", __FUNCTION__);
                rThread.SetThreadState(SMF_BaseThread::TS_DEACTIVING);
			    break;
		    }
#endif // #if defined(SMF_PO_IMPL_BY_SOCK)
            ...
            if (nSize < sizeof(MailHeader)) {
                SMF_LOG_ERR("*** ERROR: [%s()] nSize < sizeof(MailHeader)\n", __FUNCTION__);
                continue;
            }
            ...
            if (oMailHeader.nPackageSize != nSize) {
                SMF_LOG_ERR("*** ERROR: [%s()] oMailHeader.nPackageSize != nSize\n", __FUNCTION__);
                continue;
            }
