...
CG_ErrorCode CodeGen::GenerateIncHpp()
{
    ...
    sFileContent += TK_Tools::FormatStr("#define %s_INSTANCE_COUNT %lu", m_sPrefix.c_str(), m_nInstanceCount) + NEW_LINE;
...
CG_ErrorCode CodeGen::GenerateEnginesHpp()
{
    ...
    sFileContent += TK_Tools::FormatStr("    virtual SMF_ErrorCode PreProcessEvent(const SMF_EventInfo &rEventInfo, bool &bProcessed);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    virtual SMF_ErrorCode PostProcessEvent(const SMF_EventInfo &rEventInfo, bool bProcessed);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    virtual SMF_ErrorCode OnUnhandledEvent(const SMF_EventInfo &rEventInfo);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("};") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
...
CG_ErrorCode CodeGen::GenerateEnginesCpp()
{
    ...
    sFileContent += TK_Tools::FormatStr("SMF_ErrorCode %s::PreProcessEvent(const SMF_EventInfo &rEventInfo, bool &bProcessed)", CG_ENGINE_CLASS_NAME("MainEngine").c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("{") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    SMF_ErrorCode nErrorCode = SMF_ERR_OK;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    %s &rEventHandler = %s::GetInstance(GetIndex());", CG_EVENT_HANDLER_CLASS_NAME.c_str(), CG_EVENT_HANDLER_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    nErrorCode = rEventHandler.PreProcessEvent(rEventInfo, bProcessed);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    return nErrorCode;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("}") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;

    sFileContent += TK_Tools::FormatStr("SMF_ErrorCode %s::PostProcessEvent(const SMF_EventInfo &rEventInfo, bool bProcessed)", CG_ENGINE_CLASS_NAME("MainEngine").c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("{") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    SMF_ErrorCode nErrorCode = SMF_ERR_OK;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    %s &rEventHandler = %s::GetInstance(GetIndex());", CG_EVENT_HANDLER_CLASS_NAME.c_str(), CG_EVENT_HANDLER_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    nErrorCode = rEventHandler.PostProcessEvent(rEventInfo, bProcessed);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    return nErrorCode;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("}") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;

    sFileContent += TK_Tools::FormatStr("SMF_ErrorCode %s::OnUnhandledEvent(const SMF_EventInfo &rEventInfo)", CG_ENGINE_CLASS_NAME("MainEngine").c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("{") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    SMF_LOG_ENGINE();") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    SMF_ErrorCode nErrorCode = SMF_ERR_OK;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    %s &rEventHandler = %s::GetInstance(GetIndex());", CG_EVENT_HANDLER_CLASS_NAME.c_str(), CG_EVENT_HANDLER_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    nErrorCode = rEventHandler.OnUnhandledEvent(rEventInfo);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    return nErrorCode;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("}") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
...
CG_ErrorCode CodeGen::GenerateEventHandlerHpp()
{
    ...
    sFileContent += TK_Tools::FormatStr("class %s : public SMF_BaseEventHandler, public SMF_PlacementNewDelete", CG_EVENT_HANDLER_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("{") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("protected:") + NEW_LINE;
    ...
    sFileContent += TK_Tools::FormatStr("    //") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    virtual SMF_ErrorCode PreProcessEvent(const SMF_EventInfo &rEventInfo, bool &bProcessed) { bProcessed = false; return SMF_ERR_OK; }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    virtual SMF_ErrorCode PostProcessEvent(const SMF_EventInfo &rEventInfo, bool bProcessed) { return SMF_ERR_OK; }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    virtual SMF_ErrorCode OnUnhandledEvent(const SMF_EventInfo &rEventInfo) { return SMF_ERR_OK; }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
...
CG_ErrorCode CodeGen::GenerateEventHandlerImplCpp()
{
    ...
    sFileContent += TK_Tools::FormatStr("{") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("private:") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    static SMF_MACHINE_WORD_TYPE arrObjBuff[%s_INSTANCE_COUNT][SMF_ALIGNED_SIZE(%s) / sizeof(SMF_MACHINE_WORD_TYPE)];", m_sPrefix.c_str(), CG_EVENT_HANDLER_IMPL_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("public:") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    static %s *NewObject(size_t nIndex) {", CG_EVENT_HANDLER_IMPL_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("        if (nIndex >= %s_INSTANCE_COUNT) {", m_sPrefix.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("            nIndex = 0;") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("        }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("        return new(arrObjBuff[nIndex]) %s(nIndex);", CG_EVENT_HANDLER_IMPL_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    static void DeleteObject(%s *pObject) {", CG_EVENT_HANDLER_IMPL_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("        if (pObject != NULL) {") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("            pObject->~%s();", CG_EVENT_HANDLER_IMPL_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("            pObject->operator delete(pObject, pObject);") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("        }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("    }") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("};") + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("SMF_MACHINE_WORD_TYPE %s_EventHandleFactory::arrObjBuff[%s_INSTANCE_COUNT][SMF_ALIGNED_SIZE(%s) / sizeof(SMF_MACHINE_WORD_TYPE)];", m_sPrefix.c_str(), m_sPrefix.c_str(), CG_EVENT_HANDLER_IMPL_CLASS_NAME.c_str()) + NEW_LINE;
    sFileContent += TK_Tools::FormatStr("") + NEW_LINE;
    ...
    for (i = 0; i < oShapeState.arrInternalTransItems.size(); i++) {
        ...
        if (!IsValueInSet(oTransItem.sOperation, setOperations)) {
            ...
            if (!oTransItem.sOperation.empty()) {
                sFileContent += TK_Tools::FormatStr("    SMF_ErrorCode %s(SMF_OpArg &rOpArg) { SMF_LOG_STATE(); return GetEventHandler().%s(rOpArg); }", oTransItem.sOperation.c_str(), oTransItem.sOperation.c_str()) + NEW_LINE;
            }
        }
    }
