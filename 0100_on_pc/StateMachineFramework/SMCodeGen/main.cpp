...
int ParseAndGen(unsigned int nInstanceCount, const std::vector<std::string> &arrShapeXmlFileContent, bool bDump, std::vector<std::string> &arrDumpContent, 
                const std::string &sCodeOutputDir, const std::string &sCodePrifex)
{
    ...
    for (i = 0; i < arrShapeXmlFileContent.size(); i++) {
        ...
        for (j = 0; j < pShapeParser->GetShapeStates().size(); j++) {
            ...
            if (true /*pShapeState_Sub->pParentShapeState == NULL*/) {
        ...
        for (j = 0; j < pShapeParser->GetShapeTranss().size(); j++) {
            ShapeTrans *pShapeTrans = &pShapeParser->GetShapeTranss()[j];
    ...
    pCodeGen->SetInstanceCount(nInstanceCount);
...
{
    TK_MSG("Usage:\n");
    TK_MSG("  %s <instance_count> <code_prefix> <gen_src_dir> <dump_xml_path>|not_dump <shapes_xml_path> [<shapes_xml_path>] ...\n", argv[0]);
    TK_MSG("\n");
}
...
int main(int argc, char* argv[])
{
    ...
    nInstanceCount = TK_Tools::StrToUL(argv[1]);
    if (nInstanceCount == 0) {
        nInstanceCount = 1;
    }

    sCodePrifex = argv[2];
    sCodeOutputDir = argv[3];
    sDumpXmlPath = argv[4];
    arrInputFilePath.push_back(argv[5]);
    
    for (i = 6; i < (unsigned int)argc; i++) {
    ...
    nRet = ParseAndGen(nInstanceCount, arrFileContent, bDump, arrDumpContent, sCodeOutputDir, sCodePrifex);
    if (nRet != 0) {
        nRet = -1;
        goto ERR1;
    }
