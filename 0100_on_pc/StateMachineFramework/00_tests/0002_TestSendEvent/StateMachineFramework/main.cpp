...
{
    static SMF_BaseEngine &rMainEngine = APP::SMF_GET_ENGINE(MainEngine, 0);

    return rMainEngine;
}

SMF_BaseEngine &GetMainEngine21()
{
    static SMF_BaseEngine &rMainEngine = USB::SMF_GET_ENGINE(MainEngine, 0);

    return rMainEngine;
}

SMF_BaseEngine &GetMainEngine22()
{
    static SMF_BaseEngine &rMainEngine = USB::SMF_GET_ENGINE(MainEngine, 1);
...
enum {
    ...
    ENGINE_ID_ENGINE_002,
    ENGINE_ID_ENGINE_003,
};
...
void help()
{
    ...
    TK_MSG("  1|21|22: set the current engine NO.\n");
    TK_MSG("  ce|engine: show the current engine NO.\n");
    ...
    TK_MSG("  e|event <event_name>: post a event.\n");
...
void TryDeleteTempObjectFiles()
{
   ...
   if (!TK_Tools::ProcessExists("StateMachineFramework")) {
    ...
    if (!TK_Tools::ProcessExists("IPCSender")) {
        ...
        std::string sCmdLine = TK_Tools::FormatStr("rm -f %s/*", SMF_TEMP_DIR_ROOT);
        system(sCmdLine.c_str());
    } else {
...
std::string g_sEngineName = "1";
SMF_BaseEngine &GetCurrentEngine()
{
    if (g_sEngineName == "1") {
        return GetMainEngine();
    } else if (g_sEngineName == "21") {
        return GetMainEngine21();
    } else if (g_sEngineName == "22") {
...
int main(int argc, char* argv[])
{
    ...
    GetMainEngine21().AttachToEventQueue(GetMainEventQueue(), ENGINE_ID_ENGINE_002);
    GetMainEngine22().AttachToEventQueue(GetMainEventQueue(), ENGINE_ID_ENGINE_003);
    GetMainStateMachineThread().Start();
    GetMainEngine().Start();
    GetMainEngine21().Start();
    GetMainEngine22().Start();
    ...
    while (true) {
        ...
        if (vCmdLine[0] == "exit") {
            ...
            help();
            continue;
        } else if (vCmdLine[0] == "1" || vCmdLine[0] == "21" || vCmdLine[0] == "22") {
            g_sEngineName = vCmdLine[0];
            continue;
        } else if (vCmdLine[0] == "ce" || vCmdLine[0] == "current_engine") {
            TK_MSG("%s\n", g_sEngineName.c_str());
            continue;
        } else if (vCmdLine[0] == "start") {
            GetCurrentEngine().PostEvent(SMF_D_EVENT_ID(_GOTO_FIRST_CHILD));
            continue;
        } else if (vCmdLine[0] == "current_state" || vCmdLine[0] == "cs") {
            if (GetCurrentEngine().GetCurrentState() != NULL) {
                TK_MSG("%s\n", GetCurrentEngine().GetCurrentState()->GetStateName().c_str());
            } else {
            ...
            if (vCmdLine.size() >= 2) {
                unsigned int nEventId = GetCurrentEngine().GetEventIdByName(vCmdLine[1]);
                if (nEventId != SMF_D_EVENT_ID(_INVALID)) {
                    TK_MSG("<<<=== PostEvent(%s)\n", vCmdLine[1].c_str());
                    GetCurrentEngine().PostEvent(nEventId);
                    TK_MSG("===>>> PostEvent(%s)\n", vCmdLine[1].c_str());
                } else {
                    TK_ERR("*** Unknown event ID: %s\n", vCmdLine[1].c_str());
                }
            ...
            if (vCmdLine.size() >= 2) {
                ...
                if (nEventId != SMF_D_EVENT_ID(_INVALID)) {
                    TK_MSG("<<<=== SendEvent(%s)\n", vCmdLine[1].c_str());
                    GetCurrentEngine().SendEvent(nEventId);
                    TK_MSG("===>>> SendEvent(%s)\n", vCmdLine[1].c_str());
                } else {
                    TK_ERR("*** Unknown event ID: %s\n", vCmdLine[1].c_str());
                }
