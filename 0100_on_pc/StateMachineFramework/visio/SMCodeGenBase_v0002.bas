...
Sub A_000_ProcessAllPagesAndGenerateCppFiles()
    ...
    If Dir(GetOutputDir(), vbDirectory) = "" Then
...
Function GenerateCppCode() As Boolean
    ...
    If Dir(GetCurDocNameWithoutExt() + "_inc.bat", vbNormal) = "" Then
        MsgBox "Failded to create the file: " + GetCurDocNameWithoutExt() + "_inc.bat", vbCritical + vbOKOnly, "Error"
        GenerateCppCode = False
        Exit Function
    End If

    If Dir(GetCurDocNameWithoutExt() + ".bat", vbNormal) = "" Then
