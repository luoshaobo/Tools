...
private:
    static DWORD ThreadProcStatic(SMF_PlatformThreadImpl *pThis);
    DWORD ThreadProc();
...
DWORD SMF_PlatformThreadImpl::ThreadProcStatic(SMF_PlatformThreadImpl *pThis)
{
    DWORD nRet = 0;
...
DWORD SMF_PlatformThreadImpl::ThreadProc()
{
    DWORD nRet = 0;
...
SMF_ErrorCode SMF_PlatformThreadImpl::Start()
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        m_pThread = new std::thread(SMF_PlatformThreadImpl::ThreadProcStatic, this);
    }
...
{
    SMF_ErrorCode nErrorCode = SMF_ERR_NOT_IMPL;
...
SMF_PlatformThread::ThreadId SMF_PlatformThreadImpl::GetCurrentThreadId()
{
    ...
    nThreadId = std::this_thread::get_id();
