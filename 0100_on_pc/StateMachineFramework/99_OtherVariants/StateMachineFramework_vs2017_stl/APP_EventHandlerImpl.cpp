...
APP_EventHandlerImpl::APP_EventHandlerImpl(size_t nIndex)
    ...
    , nSleepValue(1)
    , nAwakeValue(2)
    , m_oCallThread("RemoteThreadCallThread")
    , m_oEventThread("RemoteThreadEventThread")
...
SMF_ErrorCode APP_EventHandlerImpl::ThreadEventQueueProc(SMF_BaseThread &rThread, QueueId nQueueId, EventData &rEventData)
{
    ...
    if (rEventData.threadEventArgs2 != 0) {
        std::shared_ptr<MyEventData> myEventData = std::dynamic_pointer_cast<MyEventData>(rEventData.threadEventArgs2);
        if (myEventData != 0) {
            int n = myEventData->n;
        }
    }
...
SMF_ErrorCode APP_EventHandlerImpl::OnThreadTimer1(SMF_OpArg &rOpArg)
{
    ...
    if (m_bTimer1Running) {
        ...
        printf("%s(): StartTimer()\n", __FUNCTION__);
        m_oTimerThread.StartTimer(TIMER_ID_TIMER_001, 3000, true);
...
SMF_ErrorCode APP_EventHandlerImpl::OnThreadTimer2(SMF_OpArg &rOpArg)
{
    ...
    m_oEventThread.PostEvent(0, NULL, ThreadEventArgs2(new MyEventData(88, "88")));
