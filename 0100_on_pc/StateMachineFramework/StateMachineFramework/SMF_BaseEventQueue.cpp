...
SMF_ErrorCode SMF_BaseEventQueue::PostEvent(EngineId nEngineId, SenderId nSenderId, EventId nEventId, ThreadEventArgs *pThreadEventArgs /*= NULL*/)
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;
    ...
    if (nErrorCode == SMF_ERR_OK) {
        nErrorCode = m_pThread->PostEvent(m_nQueueId, nEngineId, nSenderId, nEventId, pThreadEventArgs);
    }

    return nErrorCode;
}

SMF_ErrorCode SMF_BaseEventQueue::SendEvent(EngineId nEngineId, SenderId nSenderId, EventId nEventId, ThreadEventArgs *pThreadEventArgs /*= NULL*/)
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;
    ...
    if (nErrorCode == SMF_ERR_OK) {
        nErrorCode = m_pThread->SendEvent(m_nQueueId, nEngineId, nSenderId, nEventId, pThreadEventArgs);
    }
...
SMF_ErrorCode SMF_BaseEventQueue::ThreadEventQueueProc(SMF_BaseThread &rThread, QueueId nQueueId, EventData &rEventData)
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        ...
        if (pEngineEventHandlerInfo != NULL && pEngineEventHandlerInfo->bStarted) {
            nErrorCode = pEngineEventHandlerInfo->pEngineEventHandler->HandleEvent(rEventData.nSenderId, rEventData.nEventId, rEventData.pThreadEventArgs);
        }
    }
