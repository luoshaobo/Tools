/*
 * Generated by asn1c-0.9.27 (http://lionet.info/asn1c)
 * From ASN.1 module "ExpiryNotification"
 * 	found in "../schemas/asn/expiry_notification/ExpiryNotification.asn"
 * 	`asn1c -gen-PER -S`
 */

#include "Resource.h"

static asn_TYPE_member_t asn_MBR_Resource_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct Resource, name),
		(ASN_TAG_CLASS_CONTEXT | (0 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_UTF8String,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"name"
		},
	{ ATF_POINTER, 1, offsetof(struct Resource, parameters),
		(ASN_TAG_CLASS_CONTEXT | (1 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_UTF8String,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"parameters"
		},
	{ ATF_NOFLAGS, 0, offsetof(struct Resource, evictionType),
		(ASN_TAG_CLASS_CONTEXT | (2 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_EvictionType,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"evictionType"
		},
};
static int asn_MAP_Resource_oms_1[] = { 1 };
static ber_tlv_tag_t asn_DEF_Resource_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static asn_TYPE_tag2member_t asn_MAP_Resource_tag2el_1[] = {
    { (ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0 }, /* name */
    { (ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0 }, /* parameters */
    { (ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0, 0 } /* evictionType */
};
static asn_SEQUENCE_specifics_t asn_SPC_Resource_specs_1 = {
	sizeof(struct Resource),
	offsetof(struct Resource, _asn_ctx),
	asn_MAP_Resource_tag2el_1,
	3,	/* Count of tags in the map */
	asn_MAP_Resource_oms_1,	/* Optional members */
	1, 0,	/* Root/Additions */
	2,	/* Start extensions */
	4	/* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_Resource = {
	"Resource",
	"Resource",
	SEQUENCE_free,
	SEQUENCE_print,
	SEQUENCE_constraint,
	SEQUENCE_decode_ber,
	SEQUENCE_encode_der,
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	SEQUENCE_decode_uper,
	SEQUENCE_encode_uper,
	0,	/* Use generic outmost tag fetcher */
	asn_DEF_Resource_tags_1,
	sizeof(asn_DEF_Resource_tags_1)
		/sizeof(asn_DEF_Resource_tags_1[0]), /* 1 */
	asn_DEF_Resource_tags_1,	/* Same as above */
	sizeof(asn_DEF_Resource_tags_1)
		/sizeof(asn_DEF_Resource_tags_1[0]), /* 1 */
	0,	/* No PER visible constraints */
	asn_MBR_Resource_1,
	3,	/* Elements count */
	&asn_SPC_Resource_specs_1	/* Additional specs */
};

