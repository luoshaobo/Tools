...
SteadyTimestamp::SteadyTimestamp()
{
	update();
}


SteadyTimestamp::SteadyTimestamp(TimeVal tv)
{
	m_ts = tv;
}


SteadyTimestamp::SteadyTimestamp(const SteadyTimestamp& other)
{
	m_ts = other.m_ts;
}
...
SteadyTimestamp& SteadyTimestamp::operator = (const SteadyTimestamp& other)
{
	m_ts = other.m_ts;
	return *this;
}
...
SteadyTimestamp& SteadyTimestamp::operator = (TimeVal tv)
{
	m_ts = tv;
	return *this;
}
...
void SteadyTimestamp::swap(SteadyTimestamp& timestamp)
{
	std::swap(m_ts, timestamp.m_ts);
}


void SteadyTimestamp::update()
{
#if defined(WIN32)
    m_ts = GetTickCount() * 1000;
#else
    struct timespec ts = { 0, 0 };
    clock_gettime(CLOCK_BOOTTIME, &ts);
    m_ts = ts.tv_sec * resolution() + ts.tv_nsec / 1000;
#endif
}


SteadyTimestamp  SteadyTimestamp::operator +  (const TimeElapse& span) const
{
	return *this + span.getTotalMicroseconds();
}
...
SteadyTimestamp& SteadyTimestamp::operator += (const TimeElapse& span)
{
  return *this += span.getTotalMicroseconds();
}
