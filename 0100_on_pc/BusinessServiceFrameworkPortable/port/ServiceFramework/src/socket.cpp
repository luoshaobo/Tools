...
int MessagingSocket::receiveMessages(std::vector<std::string>& messages) throw (SocketException)
{
  ...
  do
  {
    ...
    if (recvBytesAvailable() > 0 || check_socket_close)
    {
      ...
      //comment unused log, Wenxin Fang 20200512
      ...
      while (m_rawbuf.get_size() > 0)
      {
        ...
        case CLEAN:
          ...
          if (m_rawbuf.get_size() >= sizeof(m_expected_message_length))
          {
            ...
            m_collected_header_length = (uint32_t)read_from_rawbuf; // just for completeness, could be done without it
          ...
          else
          {
            ...
            m_collected_header_length = (uint32_t)read_from_rawbuf;
        ...
        case FULL_HEADER:
          ...
          if (m_rawbuf.get_size() >= m_expected_message_length)
          {
            ...
            if (m_expected_message_length <= sizeof(m_messagebuf))
            {
              ...
              m_collected_message_length = (uint32_t)read_from_rawbuf; // just for completeness, not really needed for the msg any more
            }
            else
            {
          ...
          else
          {
            ...
            if (m_expected_message_length <= m_rawbuf.get_capacity())
            {
              ...
              m_collected_message_length = (uint32_t)read_from_rawbuf;
...
int MessagingSocket::sendMessage(const std::string& message)
{
  if (!message.empty())
  {
    ...
    uint32_t endianized_length = htonl((u_long)message.size());
    ...
    unsigned int msg_size = (unsigned int)message.size();
...
{
  int newConnectionFileDescriptor = (int)OS_INVALID_SOCKET;

  if ((newConnectionFileDescriptor = (int)::accept(m_socket->getSockFD(), NULL, 0)) < 0)
  {
    throw SocketException("TCPServerSocket::accept failed by accept()", true);
  }
