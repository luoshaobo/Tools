...
class Lib_API SteadyTimestamp
    /// A SteadyTimestamp stores a monotonic* time value
    /// with (theoretical) microseconds resolution.
    /// SteadyTimestamps can be compared with each other
    /// and simple arithmetics are supported.
    ///
    /// [*] Note that SteadyTimestamp values are only monotonic as
    /// long as the cpu's clock is monotonic as well
    /// (and not, e.g. set back).
    ///
{
public:
    // monotonic cpu time value in microsecond resolution
    typedef int64_t TimeVal;
    ...
    SteadyTimestamp(TimeVal tv);
        /// Creates a timestamp from the given time value.

    SteadyTimestamp(const SteadyTimestamp& other);
        /// Copy constructor.

    ~SteadyTimestamp();
        /// Destroys the timestamp

    SteadyTimestamp& operator = (const SteadyTimestamp& other);
    SteadyTimestamp& operator = (TimeVal tv);

    void swap(SteadyTimestamp& timestamp);
        /// Swaps the SteadyTimestamp with another one.

    void update();
        /// Updates the SteadyTimestamp with the current time.

    bool operator == (const SteadyTimestamp& ts) const;
    bool operator != (const SteadyTimestamp& ts) const;
    bool operator >  (const SteadyTimestamp& ts) const;
    bool operator >= (const SteadyTimestamp& ts) const;
    bool operator <  (const SteadyTimestamp& ts) const;
    ...
    SteadyTimestamp  operator +  (Difference d) const;
    SteadyTimestamp  operator +  (const TimeElapse& span) const;
    SteadyTimestamp  operator -  (Difference d) const;
    SteadyTimestamp  operator -  (const TimeElapse& span) const;
    Difference   operator -  (const SteadyTimestamp& ts) const;
    SteadyTimestamp& operator += (Difference d);
    SteadyTimestamp& operator += (const TimeElapse& span);
    SteadyTimestamp& operator -= (Difference d);
    SteadyTimestamp& operator -= (const TimeElapse& span);
    ...
    Difference elapsed() const;
        /// Returns the time elapsed since the time denoted by
        /// the timestamp. Equivalent to SteadyTimestamp() - *this.
...
// inlines
//
inline bool SteadyTimestamp::operator == (const SteadyTimestamp& ts) const
{
    return m_ts == ts.m_ts;
}
...
inline SteadyTimestamp SteadyTimestamp::operator + (SteadyTimestamp::Difference d) const
{
    return SteadyTimestamp(m_ts + d);
}
...
inline SteadyTimestamp::Difference SteadyTimestamp::operator - (const SteadyTimestamp& ts) const
{
    return m_ts - ts.m_ts;
}


inline SteadyTimestamp& SteadyTimestamp::operator += (SteadyTimestamp::Difference d)
{
    m_ts += d;
    return *this;
}
...
inline SteadyTimestamp::TimeVal SteadyTimestamp::value() const
{
    return m_ts;
}


inline SteadyTimestamp::Difference SteadyTimestamp::elapsed() const
{
    SteadyTimestamp now;
    return now - *this;
}
...
inline bool SteadyTimestamp::isElapsed(SteadyTimestamp::Difference interval) const
{
    SteadyTimestamp now;
    SteadyTimestamp::Difference diff = now - *this;
    return diff >= interval;
}


inline SteadyTimestamp::TimeVal SteadyTimestamp::resolution()
{
    return 1000000;
}


inline void swap(SteadyTimestamp& s1, SteadyTimestamp& s2)
{
    s1.swap(s2);
}
