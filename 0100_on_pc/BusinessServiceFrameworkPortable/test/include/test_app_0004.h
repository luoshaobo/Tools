#ifndef TEST_APP_0004_H
#define TEST_APP_0004_H
...
namespace test_app_0004 {
    ...
    public:
        ...
        virtual const bool OnHandleSignal(const std::shared_ptr<fsm::Signal> signal)
        {
...

            {
                ...
                if (IsSharedPtrNotNull(sig) && (sig->m_nValue == m_nHint)) {
	                BSFWK_LOG_SERVICE_GEN_PRINTF("\n");
    ...
    class EntityFactory_0002_01 : public EntityFactoryBase
    {
        typedef Context_0001 Context;

    public:
        EntityFactory_0002_01() 
            : EntityFactoryBase()
            , m_context()
        {
        }
        ...
        virtual std::shared_ptr<IServiceEntity> CreateServiceEntity(IServiceStateMachine &rServiceStateMachine)
        {
            return std::make_shared<ServiceEntity_0001>(rServiceStateMachine, m_context, std::string("Service_0002_01"), 3);
        }
        ...
        virtual std::shared_ptr<IJobEntity> CreateJobEntity(IServiceStateMachine &rServiceStateMachine, IJobStateMachine &rJobStateMachine, uint32_t nIndex)
        {
            ...
            switch (nIndex) {
                case 0:
                    {
                        pJobEntity = std::make_shared<JobEntity_0001_0001>(rServiceStateMachine, rJobStateMachine, nIndex, m_context, std::string("Job_0002_01_1"));
                    }
                    break;
                case 1:
                    {
                        pJobEntity = std::make_shared<JobEntity_0001_0002>(rServiceStateMachine, rJobStateMachine, nIndex, m_context, std::string("Job_0002_01_2"));
                    }
                    break;
                default:
                    {
        ...
        virtual const std::string GetServiceStateMachineName()
        {
            return std::string("ServiceSM_0002_01");
        }

        virtual const std::string GetJobStateMachineName(const uint32_t nIndex)
        {
            return std::string("JobSM_0002_01_") + FormatStr("%u", nIndex + 1);
        }
    ...
    class EntityFactory_0002_02 : public EntityFactoryBase
    {
        typedef Context_0001 Context;

    public:
        EntityFactory_0002_02() 
            : EntityFactoryBase()
            , m_context()
        {
        }
        ...
        virtual std::shared_ptr<IServiceEntity> CreateServiceEntity(IServiceStateMachine &rServiceStateMachine)
        {
            return std::make_shared<ServiceEntity_0001>(rServiceStateMachine, m_context, std::string("Service_0002_02"), 4);
        }
        ...
        virtual std::shared_ptr<IJobEntity> CreateJobEntity(IServiceStateMachine &rServiceStateMachine, IJobStateMachine &rJobStateMachine, uint32_t nIndex)
        {
            ...
            switch (nIndex) {
                case 0:
                    {
                        pJobEntity = std::make_shared<JobEntity_0001_0001>(rServiceStateMachine, rJobStateMachine, nIndex, m_context, std::string("Job_0002_02_1"));
                    }
                    break;
                case 1:
                    {
                        pJobEntity = std::make_shared<JobEntity_0001_0002>(rServiceStateMachine, rJobStateMachine, nIndex, m_context, std::string("Job_0002_02_2"));
                    }
                    break;
            }
        ...
        virtual const std::string GetServiceStateMachineName()
        {
            return std::string("ServiceSM_0002_02");
        }

        virtual const std::string GetJobStateMachineName(const uint32_t nIndex)
        {
            return std::string("JobSM_0002_02_") + FormatStr("%u", nIndex + 1);
        }
