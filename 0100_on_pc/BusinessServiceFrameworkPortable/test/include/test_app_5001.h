#ifndef TEST_APP_5001_H
#define TEST_APP_5001_H

#include "test_app_global.h"

namespace test_app_5001 {
    ...
    public:
			...
			, m_cyclicalRequestConfig()
        {
            ...
            m_retryConfig.GetTimeoutRetryInfo().SetRetryCount(5);
            m_retryConfig.GetTimeoutRetryInfo().SetTimeout(10000);

            m_retryConfig.GetFailureRetryInfo().SetEnabled(true);
            m_retryConfig.GetFailureRetryInfo().SetRetryCount(5);
            m_retryConfig.GetFailureRetryInfo().SetDelayTime(1000);

            m_retryConfig.SetDuration(30000);
			
			m_cyclicalRequestConfig.SetEnabled(true);
			m_cyclicalRequestConfig.SetInterval(1000);
        }
        ...
        virtual const bool OnHandleSignal(const std::shared_ptr<fsm::Signal> signal)
        {
...

            {
                ...
                if (IsSharedPtrNotNull(sig)) {
                    ...
                    GetServiceStateMachine().StartCyclicRequest();
                }
            }
        ...
        {
            BSFWK_LOG_SERVICE_GEN_PRINTF("\n");
            return m_cyclicalRequestConfig;
        }
    ...
    public:
            ...
            , m_delayConfig()
        {
            ...
            m_retryConfig.GetTimeoutRetryInfo().SetTimeout(3000);
            
            m_retryConfig.GetFailureRetryInfo().SetEnabled(true);
            m_retryConfig.GetFailureRetryInfo().SetRetryCount(3);
            m_retryConfig.GetFailureRetryInfo().SetDelayTime(1000);

            m_retryConfig.SetDuration(8000);
            
            m_delayConfig.SetEnabled(true);
            m_delayConfig.SetDelayTime(2000);
        }
        ...
        virtual void OnJobBegin()
        {
            ...
            if ((i % 8) != 0) {
                m_apiSimulatorTimer = GetJobStateMachine().StartTimer(4000, EVENT_API_CALL_RESPONSE);
            } else {
    ...
    public:
        ...
        {
            BSFWK_LOG_JOB_GEN_PRINTF("\n");
            m_apiSimulatorTimer = GetJobStateMachine().StartTimer(1000, EVENT_API_CALL_RESPONSE);
        }
