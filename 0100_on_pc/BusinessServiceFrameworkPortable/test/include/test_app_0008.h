#ifndef TEST_APP_0008_H
#define TEST_APP_0008_H
...
namespace test_app_0008 {
    ...
    public:
        ...
        virtual const bool OnHandleSignal(const std::shared_ptr<fsm::Signal> signal)
        {
            ...
            {
                std::shared_ptr<ServiceResumeSignal> sig = std::dynamic_pointer_cast<ServiceResumeSignal>(signal);
                if (IsSharedPtrNotNull(sig)) {
                    BSFWK_LOG_SERVICE_GEN_PRINTF("--- ServiceResumeSignal ---\n");
                    GetServiceStateMachine().Resume();
                    bConsumed = true;
                }
            }
    ...
    class JobEntity_0001_0001 : public JobEntityBase
    {
        ...
        enum {
            ...
            EVENT_PAUSE_SERVICE_TIMER,
            EVENT_RESUME_SERVICE_TIMER,
        };
    ...
    public:
        ...
        virtual void OnBSEvent(const std::shared_ptr<BSEvent> event)
        {
            ...
            if (IsSharedPtrNotNull(event)) {
                ...
                } else if (event->GetEventIndex() == EVENT_PAUSE_SERVICE_TIMER) {
                    static bool bFirstTime = true;
                    if (bFirstTime) {
                        GetServiceStateMachine().Pause();
                        bFirstTime = false;
                    }
                } else {
                }
        ...
        {
            BSFWK_LOG_JOB_GEN_PRINTF("\n");
            m_apiSimulatorTimer = GetJobStateMachine().StartTimer(8000, EVENT_API_CALL_RESPONSE);
            m_pauseServiceTimer = GetJobStateMachine().StartTimer(1000, EVENT_PAUSE_SERVICE_TIMER);
        }
        ...
        virtual void OnJobEnd()
        {
            ...
            GetJobStateMachine().StopTimer(m_pauseServiceTimer);
        }
    ...
    private:
        ...
        BSTimer m_pauseServiceTimer;
        BSTimer m_resumeServiceTimer;
    };
