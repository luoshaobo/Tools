...
enum SMF_EngineFeature {
    ...
    SMF_EF_PROCESS_EVENT_IN_PARENT_STATE                    = 0x00000001,
};
...
#define SMF_DECLARE_ENGINE(engine_name) \
    SMF_D_ENGINE_CLASS(engine_name) &SMF_GetEngine_ ## engine_name();

#define SMF_DEFINE_ENGINE(engine_name) \
    SMF_D_ENGINE_CLASS(engine_name) &SMF_GetEngine_ ## engine_name() \
    { \
        static SMF_D_ENGINE_CLASS(engine_name) SMF_D_ENGINE_INSTANCE(engine_name) (#engine_name); \
        return SMF_D_ENGINE_INSTANCE(engine_name); \
    }

#define SMF_GET_ENGINE(engine_name) \
    SMF_GetEngine_ ## engine_name()

#define SMF_DECLARE_STATE(state_name) \
    SMF_D_STATE_CLASS(state_name) &SMF_GetState_ ## state_name();

#define SMF_DEFINE_STATE(engine_name,state_name) \
    SMF_D_STATE_CLASS(state_name) &SMF_GetState_ ## state_name() \
    { \
        static SMF_D_STATE_CLASS(state_name) SMF_D_STATE_INSTANCE(state_name) (SMF_GET_ENGINE(engine_name), #state_name); \
        return SMF_D_STATE_INSTANCE(state_name); \
    }

#define SMF_GET_STATE(state_name) \
    SMF_GetState_ ## state_name()

#define SMF_STATE_NAME_EQUAL_TO(state_instance,state_name) \
    (state_instance).IsEqualTo(SMF_GET_STATE(state_name))

#define SMF_SET_STATE_MACHINE_TO_ENGINE(state_name,engine_name) \
    do { \
        SMF_GET_ENGINE(engine_name).SetStateMachine(&SMF_GET_STATE(state_name)); \
    } while (0)

#define SMF_ADD_STATE_TO_PARENT_STATE(state_name,parent_state_name) \
    do { \
        SMF_GET_STATE(state_name).SetParentState(&SMF_GET_STATE(parent_state_name)); \
    } while (0)
...
#define SMF_DEFINE_STATE_TRANS_ITEM(event_id,condition,operation,dest_state_name) \
        if (SMF_D_EVENT_ID(event_id) == nEventId && (condition)) { \
            ...
            oTransItem.pDestState = &SMF_GET_STATE(dest_state_name); \
