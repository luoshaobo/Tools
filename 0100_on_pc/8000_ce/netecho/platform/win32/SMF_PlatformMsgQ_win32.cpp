...
SMF_ErrorCode SMF_PlatformMsgQImpl::PostMsg(const SMF_PlatformMsgQ::Msg &rMsg)
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        bSuc = ::PostMessage(m_hWnd, rMsg.nMsgId + WM_USER, rMsg.wParam, rMsg.lParam);
        if (!bSuc) {
...
SMF_ErrorCode SMF_PlatformMsgQImpl::SendMsg(const SMF_PlatformMsgQ::Msg &rMsg)
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        ::SendMessage(m_hWnd, rMsg.nMsgId + WM_USER, rMsg.wParam, rMsg.lParam);
    }
...
std::string SMF_PlatformMsgQImpl::GetWindowClassName() const 
{
    ...
    sResult = TK_Tools::FormatStr("SMF_PlatformMsgQImpl_WindowClassName_%08X_%s", m_nSerialNo, m_sName.c_str());
...
std::string SMF_PlatformMsgQImpl::GetWindowName() const
{
    ...
    sResult = TK_Tools::FormatStr("SMF_PlatformMsgQImpl_WindowName_%08X_%s", m_nSerialNo, m_sName.c_str());
