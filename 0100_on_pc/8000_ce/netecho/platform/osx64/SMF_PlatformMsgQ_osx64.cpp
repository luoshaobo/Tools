...
private:
    struct MqMsg {
        MqMsg() : mtype(1), oMsg(Msg()), pEvent(NULL) {}
        MqMsg(Msg a_oMsg, SMF_PlatformEvent *a_pEvent = NULL)
            : mtype(1), oMsg(a_oMsg), pEvent(a_pEvent) {}
...
public:
    ...
    SMF_ErrorCode SendMsg(const SMF_PlatformMsgQ::Msg &rMsg, SMF_PlatformEvent *pWaitedEvent);
...
SMF_ErrorCode SMF_PlatformMsgQImpl::DestroyMq()
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        if (m_bMqValid) {
            nRet = ::msgctl(m_nMq, IPC_RMID, NULL);
            if (nRet != 0) {
...
SMF_ErrorCode SMF_PlatformMsgQImpl::SendMsg(const SMF_PlatformMsgQ::Msg &rMsg, SMF_PlatformEvent *pWaitedEvent)
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;
    int nRet;
    ...
    if (nErrorCode == SMF_ERR_OK) {
        MqMsg mqmsg(rMsg, pWaitedEvent != NULL ? pWaitedEvent : pEvent);
        nRet = ::msgsnd(m_nMq, reinterpret_cast<const void *>(&mqmsg), sizeof(MqMsg) - sizeof(long), 0);
        if (nRet != 0) {
            nErrorCode = SMF_ERR_PLATFORM_API_CALL_FAILED;
        }
...
SMF_ErrorCode SMF_PlatformMsgQImpl::EnterMsgLoop()
{
    ...
    if (nErrorCode == SMF_ERR_OK) {
        while (true) {
            ...
            do {
                nRet = ::msgrcv(m_nMq, reinterpret_cast<void *>(&mqmsg), sizeof(MqMsg) - sizeof(long), 0, 0);
...
std::string SMF_PlatformMsgQImpl::GetMqKeyPath() const
{
    ...
    sResult = TK_Tools::FormatStr("%s/SMF_PlatformMsgQImpl_mq_key_%s", SMF_TEMP_KEY_DIR_ROOT, m_sName.c_str());
...
SMF_ErrorCode SMF_PlatformMsgQ::SendMsg(const Msg &rMsg)
{
    ...
    if (m_pImpl != NULL) {
        nErrorCode = m_pImpl->SendMsg(rMsg, NULL);
    } else {
        nErrorCode = SMF_ERR_NULL_PTR;
    }
...
SMF_ErrorCode SMF_PlatformMsgQ::SendMsg(const Msg &rMsg, SMF_PlatformEvent *pWaitedEvent)
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;

    if (m_pImpl != NULL) {
        nErrorCode = m_pImpl->SendMsg(rMsg, pWaitedEvent);
    } else {
        nErrorCode = SMF_ERR_NULL_PTR;
    }
