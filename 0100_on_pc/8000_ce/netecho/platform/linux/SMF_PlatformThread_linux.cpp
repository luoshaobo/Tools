...
SMF_ErrorCode SMF_PlatformThreadImpl::StartThread()
{
...
SMF_ErrorCode SMF_PlatformThreadImpl::StopThread()
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;
...
bool SMF_PlatformThreadImpl::IsThreadAlive()
{
...
SMF_ErrorCode SMF_PlatformThread::StartThread()
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;

    if (m_pImpl != NULL) {
        nErrorCode = m_pImpl->StartThread();
    } else {
...
SMF_ErrorCode SMF_PlatformThread::StopThread()
{
    SMF_ErrorCode nErrorCode = SMF_ERR_OK;

    if (m_pImpl != NULL) {
        nErrorCode = m_pImpl->StopThread();
    } else {
        nErrorCode = SMF_ERR_NULL_PTR;
    }
...
bool SMF_PlatformThread::IsThreadAlive()
{
    bool bThreadAlive = false;

    if (m_pImpl != NULL) {
        bThreadAlive = m_pImpl->IsThreadAlive();
    }
