...
//////////////////////////////////////////////////////////////////////////////////////////
// non-complete declaration for some classes
//
class entHmiBLAdaptor
{
public:
    static void postEvent(UINT16_T evtID, INT16_T value, UINT_T modifiers);
};

class vxtBaseClsGUIScreen
{
public:
    UINT16_T getScreenID() const;
};

class vxtClsGUIScreen : public vxtBaseClsGUIScreen
{
public:
    
};

class entHmiHMIManager
{
    friend void TK_Tools::entHmiHMIManager_clearScreenStackExceptingCurrent();
public:
    static entHmiHMIManager& getInstance();
    void showScreen(UINT16_T screenID);
    void closeScreen(UINT16_T screenID);
    void showPopup(UINT16_T screenID);
    void closePopup(UINT16_T screenID);
    void showAppScreen(UINT8_T appID);
    void showLastScreen();
    void showLastScreenByID(UINT16_T screenID);
    unsigned short getCurrentScreenID() const;
    void registerGenericGUI(MyGenericGUI_FuncType genericGUI);
    vxtClsGUIScreen* getCurrentPopup() const;
    void showNowPlaying();
    
private: // NOTE: the data members must match the ones in real class entHmiHMIManager completely!
    unsigned int mPopupLevel;
    void *mSourceGetter;
    void *mSourceSetter;
    void *mGenericGUI;
    std::list<vxtClsGUIScreen*> mScreenStack;
    std::list<vxtClsGUIScreen*> mWidgetStack;
    std::list<vxtClsGUIScreen*> mPopupStack;
    std::map<unsigned short,vxtClsGUIScreen*> mScreenList;
};

class hmiNavAdaptor
{
public:
    static hmiNavAdaptor& getInstance() {
        static hmiNavAdaptor navAdaptor;
        return navAdaptor;
    }
    void handleUMSG(UINT_T evt, const UINT8_T* pData, UINT_T size);
};

class hmiImageManager
{
public:
    enum Theme
    {
    ...
    static hmiImageManager& getInstance();
    BOOL_T setTheme(Theme theme);
};

class vxtRTUpdateSubject
{
public:
    void vUpdateObservers();
};

class vxtClsImageManager
{
public:
    static vxtRTUpdateSubject& rGetGlobalSubject();
};
...
extern void entISysConfiguration_SelLanguage(hmiSelLanguage lang);
class vxtClsLanguageCfg
{
public:
    enum EnumValues
    {
    ...
    static void updateLangId(UINT_T id);
};

namespace Configuration
{
    void writeScreenGrpID(UINT8_T lastStoredGrpID);
};
...
{
    entHmiBLAdaptor::postEvent(evtID, value, modifiers);
}

void entHmiHMIManager_showScreen(UINT16_T screenID)
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.showScreen(screenID);
}

void entHmiHMIManager_closeScreen(UINT16_T screenID)
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.closeScreen(screenID);
}

void entHmiHMIManager_showPopup(UINT16_T screenID)
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.showPopup(screenID);
}

void entHmiHMIManager_closePopup(UINT16_T screenID)
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.closePopup(screenID);
}
...
void entHmiHMIManager_CloseAllPopups()
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    while (true) {
        vxtClsGUIScreen* pGUIScreen = hmiMgr.getCurrentPopup();
        if (pGUIScreen == NULL) {
            break;
        } else {
            UINT16_T nScreenId = pGUIScreen->getScreenID();
            hmiMgr.closePopup(nScreenId);
        }
    }
}
...
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.showAppScreen(appID);
}

void entHmiHMIManager_showLastScreen()
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.showLastScreen();
}

void entHmiHMIManager_showLastScreenByID(UINT16_T screenID)
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.showLastScreenByID(screenID);
}

void entHmiHMIManager_registerGenericGUI(MyGenericGUI_FuncType genericGUI)
{
    entHmiHMIManager &hmiMgr = entHmiHMIManager::getInstance();
    hmiMgr.registerGenericGUI(genericGUI);
}

void entHmiHMIManager_clearScreenStackExceptingCurrent()
{
    entHmiHMIManager &rHmiHMIManager = entHmiHMIManager::getInstance();
    std::list<vxtClsGUIScreen *> &screenStack = rHmiHMIManager.mScreenStack;
	
    std::list<vxtClsGUIScreen *>::iterator it = screenStack.begin();
    if (it != screenStack.end()) {
        ++it;
        if (it != screenStack.end()) {
            screenStack.erase(it, screenStack.end());
        }
    }
}
...
{
    entHmiHMIManager &rHmiHMIManager = entHmiHMIManager::getInstance();
    rHmiHMIManager.showNowPlaying();
}

void hmiNavAdaptor_handleUMSG(UINT32_T evt, const UINT8_T* pData, UINT32_T size)
{
    hmiNavAdaptor &hmiNavAdp = hmiNavAdaptor::getInstance();
    hmiNavAdp.handleUMSG(evt, pData, size);
}
...
BOOL_T hmiImageManager_setTheme(UINT8_T theme)
{
    ...
    hmiImageManager &imgMgr = hmiImageManager::getInstance();
    ret = imgMgr.setTheme((hmiImageManager::Theme)theme);
    vxtClsImageManager::rGetGlobalSubject().vUpdateObservers();
...
{
    Configuration::writeScreenGrpID(lastStoredGrpID);
}

void SetSystemLanguage(UINT8_T lang)
{
    hmiSelLanguage hsLang;
    
    switch (lang) {
    case 0:
        hsLang = CONFIG_LANG_ENGLISH;
        break;
        
    case 1:
    default:
        hsLang = CONFIG_LANG_CHINESE;
        break;
    }
    
    ::entISysConfiguration_SelLanguage(hsLang);
    //vxtClsLanguageCfg::updateLangId(hsLang);
    entHmiBLAdaptor_postEvent(2560/*SYS_CFG_CURRENT_LANG_ENUM*/, 0, hsLang);
}
