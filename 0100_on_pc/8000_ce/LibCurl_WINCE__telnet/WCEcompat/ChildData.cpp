...
void ChildData::setCurrentDirectory(const TCHAR* directory)
{
	if (currentDirectory != NULL)
		free(currentDirectory);
	currentDirectory = (TCHAR*) ts_strdup((unsigned short*)directory);
}
...
bool ChildData::addEnvironmentVar(WCHAR* env)
{
	...
	if (name == NULL)
		goto cleanup;
	ts_strncpy( (unsigned short*)name, (unsigned short*)env, nameLen);
	name[nameLen] = '\0';
	value = (TCHAR*)malloc((ts_strlen( (unsigned short*)equals +1 )+1)*sizeof(TCHAR));
	if (value == NULL)
		goto cleanup;
	ts_strcpy((unsigned short*)value, (unsigned short*)equals+1);
...
bool ChildData::addEnvironmentList(WCHAR* env)
{
	...
	while (*(char*)env != '\0' || *(((char*)env)+1) != '\0')
	{
		...
		env += ts_strlen((unsigned short*)env)+1;
	}
	return true;
}
...
bool ChildData::restoreEnvironment()
{
	...
	for (int i=0; i<environmentLength; i++)
	{
		ts_strcpy((unsigned short*)buf, (unsigned short*)environment[i].name);
		ts_strcat((unsigned short*)buf, "=");
		ts_strcat((unsigned short*)buf, (unsigned short*)environment[i].value);
		//_tputenv(buf);
	}
	return true;
}
...
{
	int	currentDirectorySize = (currentDirectory == NULL) ? sizeof(WCHAR) : ts_strsizez((unsigned short*)currentDirectory);
	int	redirSize = 0;
	for (int i=0; i<numRedirArgs; i++)
	{
			...
			redirArgs[i].redirType == RT_FILE)
		{
			if (redirArgs[i].filename != NULL)
				redirSize += ts_strsizez((unsigned short*)redirArgs[i].filename);
		}
	}
	DWORD	envSize = 0;
	if (environment != NULL)
	...
	if (childData != NULL)
	{
		...
		{		//???? prima era (WCHAR*)p
			ts_strcpy((unsigned short*)p, (unsigned short*)currentDirectory);
			p += (ts_strlen(p) + 1) * 2;
		}
		// num redirs
		...
		for (int i=0; i<numRedirArgs; i++)
		{
			...
			if (filenamePresent)
			{		//??? Prima era (WCHAR*)p
				ts_strcpy((unsigned short*)p, (unsigned short*)redirArgs[i].filename);
					//??? Prima era (WCHAR*)p
				p += (ts_strlen((unsigned short*)p) + 1) * 2;
			}
		}
		// environment size
		...
		if (envSize != 0)
		{
			for (int i=0; i<environmentLength; i++)
			{		//??? Prima era (WCHAR*)p
				ts_strcpy((unsigned short*)p, (unsigned short*)environment[i].name);
					//??? Prima era (WCHAR*)p
				p += ts_strlen((unsigned short*)p) * sizeof(WCHAR);

				ts_strcpy((unsigned short*)p, (unsigned short*)L"=");
				p += sizeof(WCHAR);
				ts_strcpy((unsigned short*)p, (unsigned short*)environment[i].value);
				p += (ts_strlen((unsigned short*)p) + 1) * sizeof(WCHAR);
			}
...
//		return false;
	...
	strLen = ts_strlen((unsigned short*)p);
	...
	for (int i=0; i<numRedirArgs; i++)
	{
		...
		if (filenamePresent)
		{
			int strLen = ts_strlen((unsigned short*)p);
			if ((strLen+1)*2 > (int)size)
				goto cleanup;
