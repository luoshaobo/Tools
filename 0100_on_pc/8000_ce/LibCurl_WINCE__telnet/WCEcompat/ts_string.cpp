...
{
	char* result = (char*)malloc(wcslen((wchar_t*)str)+1);
	if (result == NULL)
		return NULL;
	unicode2ascii((WCHAR*)str, result);
	return result;
}
...
unsigned short* ts_strdup_ascii_to_unicode(const char* str)
{
	...
	if (result == NULL)
		return NULL;
	ascii2unicode(str, (WCHAR*)result);
	return result;
}
