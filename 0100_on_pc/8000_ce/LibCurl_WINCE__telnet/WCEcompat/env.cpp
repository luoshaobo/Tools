...
int _putenv(const char* str)
{
	...
	_wenviron[num_env_vars-1] = (wchar_t*) ts_strdup_ascii_to_unicode(str);
	return 0;
}
...
int _wputenv(const wchar_t* str)
{
	...
	if (_wenviron == NULL)
		return -1;
	num_env_vars++;
	_environ[num_env_vars-1] = ts_strdup_unicode_to_ascii( (unsigned short*)str);
	_wenviron[num_env_vars-1] = (wchar_t*) ts_strdup((unsigned short*)str);
	return 0;
}
