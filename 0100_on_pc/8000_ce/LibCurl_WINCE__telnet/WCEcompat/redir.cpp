...
//				fds->error = TRUE;
	...
	} else if (fd == 0) { // modified by luoshaobo
	    char *p = (char *)buffer;
	    if (count == 1) {
	        int nCh;
            nCh = getchar();
...
//			fds->error = TRUE;
    ...
    else if (fd == 1 || fd == 2) { // modified by luoshaobo
        int i;
        char *p = (char *)buffer;
        for (i = 0; i < count; i++) {
            putchar(p[i]);
...
//NB: modified by Jacopo to solve conflict problems: it was _fileno()
//
...
int _setmode_int(int fd, int mode)
{
	_FD_STRUCT*	fds;
	int			prevMode;
