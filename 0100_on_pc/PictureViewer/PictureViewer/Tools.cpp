...
bool LoadFile(const std::string &sFilePath, std::vector<unsigned char> &vFileContent)
{
    ...
    unsigned char *pFileContentBuf = NULL;
    AutoFreeArrayPtr<unsigned char> afap;
    size_t nRead;
    ...
    fseek(pFile, 0, SEEK_END);
    nFileSize = ftell(pFile);
    pFileContentBuf = new unsigned char[nFileSize];
    TK_ASSERT(pFileContentBuf != NULL);
    afap.Set(pFileContentBuf);
    ...
    vFileContent.clear();
    vFileContent = std::vector<unsigned char>(pFileContentBuf, pFileContentBuf + nFileSize);
...
std::string GetExtFileName(const std::string &sFilePath)
{
    ...
    const char *p = strrchr(sFilePath.c_str(), '.');
    if (p != NULL) {
        p++;
        sExtFileName = p;
    }
