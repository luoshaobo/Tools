...
extern bool SNSSIM_ProcessCmdLine(const char *pCmdLine);
...
#ifdef WIN32
    bool TKDS_ProcessCmdLineDef(const char *pCmdLine) { TK_UNUSED_VAR(pCmdLine); return false; }
    #pragma comment(linker, "/alternatename:?NAVSIM_ProcessCmdLine@@YA_NPBD@Z=?TKDS_ProcessCmdLineDef@@YA_NPBD@Z")
    ...
    bool NAVSIM_ProcessCmdLine(const char *pCmdLine) __attribute__((weak, alias("TKDS_ProcessCmdLineDef")));
    bool TLMSIM_ProcessCmdLine(const char *pCmdLine) __attribute__((weak, alias("TKDS_ProcessCmdLineDef")));
    bool AVSIM_ProcessCmdLine(const char *pCmdLine) __attribute__((weak, alias("TKDS_ProcessCmdLineDef")));
    bool SYSCONFSIM_ProcessCmdLine(const char *pCmdLine) __attribute__((weak, alias("TKDS_ProcessCmdLineDef")));
    bool SNSSIM_ProcessCmdLine(const char *pCmdLine) __attribute__((weak, alias("TKDS_ProcessCmdLineDef")));
#endif // #ifdef WIN32
...
#else
    pthread_t pth = 0;
    pthread_create(&pth, NULL, &ServiceThreadProcWin, this);
#endif // #ifdef WIN32
}
...
#else
void *DebugServer::ServiceThreadProc()
#endif // #ifdef WIN32
{
...
#else
void *DebugServer::ServiceThreadProcWin(void *arg)
#endif // #ifdef WIN32
{
