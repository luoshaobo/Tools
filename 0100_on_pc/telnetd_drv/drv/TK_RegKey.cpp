...
bool ParseKeyValueStr_binary(const std::string &sValuePart, RegKey::KeyValue &rKeyValue)
{
    bool bRet = true;
    ...
    if (bRet) {
        rKeyValue = RegKey::KeyValue(RegKey::KeyValue::KVT_BINARY, arrUnsignedCharArray);
    }
    
    return bRet;
}

bool ParseKeyValueStr_dword(const std::string &sValuePart, RegKey::KeyValue &rKeyValue)
{
    bool bRet = true;
    std::string sValuePartTemp = sValuePart;
    ...
    if (bRet) {
        rKeyValue = RegKey::KeyValue(RegKey::KeyValue::KVT_DWORD, nUnsignedIntValue);
    }
    
    return bRet;
}

bool ParseKeyValueStr_sz(const std::string &sValuePart, RegKey::KeyValue &rKeyValue)
{
    bool bRet = true;
    std::string sValuePartTemp = sValuePart;
    ...
    if (bRet) {
        rKeyValue = RegKey::KeyValue(RegKey::KeyValue::KVT_SZ, TK_Tools::str2wstr(sValuePartTemp));
    }
    
    return bRet;
}

bool ParseKeyValueStr_expand_sz(const std::string &sValuePart, RegKey::KeyValue &rKeyValue)
{
    bool bRet = true;
    std::string sValuePartTemp = sValuePart;
    ...
    if (bRet) {
        rKeyValue = RegKey::KeyValue(RegKey::KeyValue::KVT_EXPAND_SZ, TK_Tools::str2wstr(sValuePartTemp));
    }
    
    return bRet;
}

bool ParseKeyValueStr_multi_sz(const std::string &sValuePart, RegKey::KeyValue &rKeyValue)
{
    bool bRet = true;
    std::string sValuePartTemp = sValuePart;
    ...
    if (bRet) {
        ...
        if (TK_Tools::CompareNoCaseRightLen(sKeyValueStrTemp, "hex") == 0) {
            ...
            if (TK_Tools::CompareNoCaseRightLen(sKeyValueStrTemp, ":") == 0) {
                ...
                sKeyValueStrTemp = TK_Tools::TrimLeft(sKeyValueStrTemp);
                bRet = ParseKeyValueStr_binary(sKeyValueStrTemp, rKeyValue);
            }
            else {
                bRet = false;
            }
            ...
            if (TK_Tools::CompareNoCaseRightLen(sKeyValueStrTemp, ":") == 0) {
                sKeyValueStrTemp = sKeyValueStrTemp.substr(::strlen(":"));
                sKeyValueStrTemp = TK_Tools::TrimLeft(sKeyValueStrTemp);
                bRet = ParseKeyValueStr_dword(sKeyValueStrTemp, rKeyValue);
            }
            else {
                bRet = false;
            }
            ...
            if (TK_Tools::CompareNoCaseRightLen(sKeyValueStrTemp, ":") == 0) {
                sKeyValueStrTemp = sKeyValueStrTemp.substr(::strlen(":"));
                sKeyValueStrTemp = TK_Tools::TrimLeft(sKeyValueStrTemp);
                bRet = ParseKeyValueStr_expand_sz(sKeyValueStrTemp, rKeyValue);
            }
            else {
                bRet = false;
            }
            ...
            if (TK_Tools::CompareNoCaseRightLen(sKeyValueStrTemp, ":") == 0) {
                sKeyValueStrTemp = sKeyValueStrTemp.substr(::strlen(":"));
                sKeyValueStrTemp = TK_Tools::TrimLeft(sKeyValueStrTemp);
                bRet = ParseKeyValueStr_multi_sz(sKeyValueStrTemp, rKeyValue);
            }
            else {
                bRet = false;
            }
...
RegKey::ErrCode RegKey::CreateKey(RegKey **ppRetKey, const std::wstring &sKeyPath, const RegKey *pParentKey /*= NULL*/)
{
    ...
    if (nErrCode == EC_OK) {
        ...
        for (i = 0; i < sPathNodes.size(); i++) {
            ...
            nRet = ::RegCreateKeyEx(hCurentKey, sPathNode.c_str(), NULL, NULL, REG_OPTION_NON_VOLATILE, 0, NULL, &hNewKey, &dwDisposition);
...
RegKey::ErrCode RegKey::OpenKey(RegKey **ppRetKey, const std::wstring &sKeyPath, const RegKey *pParentKey /*= NULL*/)
{
    ...
    if (nErrCode == EC_OK) {
        ...
        for (i = 0; i < sPathNodes.size(); i++) {
            std::wstring &sPathNode = sPathNodes[i];
            nRet = ::RegOpenKeyEx(hCurentKey, sPathNode.c_str(), 0, 0, &hNewKey);
            if (nRet != ERROR_SUCCESS) {
...
RegKey::ErrCode RegKey::CreateKey(RegKey **ppRetKey, const std::wstring &sKeyName)
{
    ...
    if (nErrCode == EC_OK) {
        nRet = ::RegCreateKeyEx(m_hKey, sKeyName.c_str(), 0, NULL, REG_OPTION_NON_VOLATILE, 0, NULL, &hkResult, &dwDisposition);
        if (nRet != ERROR_SUCCESS) {
            nErrCode = EC_FAILED_TO_CREATE_KEY;
        }
    }
...
RegKey::ErrCode RegKey::OpenKey(RegKey **ppRetKey, const std::wstring &sKeyName)
{
    ...
    if (nErrCode == EC_OK) {
        nRet = ::RegOpenKeyEx(m_hKey, sKeyName.c_str(), 0, 0, &hkResult);
        if (nRet != ERROR_SUCCESS) {
            nErrCode = EC_FAILED_TO_OPEN_KEY;
        }
    }
...
RegKey::ErrCode RegKey::ParseKeyPath(const std::wstring &sKeyPath, const RegKey *pParentKey, std::vector<std::wstring> &sPathNodes, HKEY &hParentKey)
{
    ...
    if (nErrCode == EC_OK) {
        token = wcstok((wchar_t *)sKeyPath.c_str(), seps);
        while (token != NULL) {
