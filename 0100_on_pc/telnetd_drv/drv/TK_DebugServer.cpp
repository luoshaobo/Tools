...
#define PORT_BASE				3721

extern bool DS_ProcessCmdLine(TK_Tools::DebugServerInstanceId id, const char *pCmdLine);

namespace TK_Tools {

DebugServer& DebugServer::GetInstance(DebugServerInstanceId id /*= DSIID_DEF*/)
{
    static DebugServer *pDebugServers[DSIID_COUNT] = { NULL };

    if (id > DSIID_COUNT) {
        id = DSIID_DEF;
    }
    
    if (pDebugServers[id] == NULL) {
        pDebugServers[id] = new DebugServer(PORT_BASE + id, &DS_ProcessCmdLine);
    }

    return *pDebugServers[id];
}
        ...
        if (CompareNoCase(buf, "open console 1") == 0) {
            ...
            if (CompareNoCase(buf, "~!@#$magic%^&*(") == 0) {
                ...
                if (m_pProcessCmdLineFunc != NULL) {
                    if (m_pProcessCmdLineFunc((DebugServerInstanceId)(m_nServerPort - PORT_BASE), buf)) {
                        // do nothing
                    } else {
                        // do nothing
                    }
