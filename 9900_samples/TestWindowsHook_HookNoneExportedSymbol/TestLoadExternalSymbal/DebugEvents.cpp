...
#define _ASSERTE(n)    \
    do { \
        printf("*** FATAL ERROR: assert in [%s: %s(): %u]: %s\n", __FILE__, __FUNCTION__, __LINE__, ##n); \
        exit(0); \
    } while (0)
//#define _tprintf(...)
#else
...
int debuger_tmain( int argc, TCHAR* argv[] )
{
	...
	else if( _tcscmp( argv[1], ccAttach ) == 0 )
	{
		...
		DWORD ProcessId = _tcstoul( argv[2], NULL, 10);
...
//
extern void SetForInjectionBegin();
HANDLE g_hDebugeeProcess = NULL;
DWORD g_nDebugeeProcessId = 0;
BOOL g_bDebugActiveProcessStop = FALSE;
static CREATE_PROCESS_DEBUG_INFO g_CreateProcessInfo;
bool DebugLoop( DWORD Timeout )
{
	...
	while( bContinue ) 
	{
        ...
        if (g_bDebugActiveProcessStop && g_nDebugeeProcessId != 0) {
            BOOL bSuc = DebugActiveProcessStop(g_nDebugeeProcessId);
            if (!bSuc) {
                _ASSERTE( !_T("DebugActiveProcessStop(g_nDebugeeProcessId) failed.") );
            } else {
                _tprintf(TEXT("Debugger has detatched from the debuggee!\n"));
            }
            return true;
        }
		...
		if( WaitForDebugEvent( &DebugEvent, Timeout ) )
		{
			...
			case CREATE_PROCESS_DEBUG_EVENT:
                ...
                memcpy(&g_CreateProcessInfo, &DebugEvent.u.CreateProcessInfo, sizeof(CREATE_PROCESS_DEBUG_INFO));
                ...
                g_hDebugeeProcess = DebugEvent.u.CreateProcessInfo.hProcess;
                g_nDebugeeProcessId = DebugEvent.dwProcessId;
                SetForInjectionBegin();
				break;
