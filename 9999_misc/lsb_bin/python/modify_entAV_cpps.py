...
"""
For uc8_t entIAV_GetAudioSettingsAvailability(), please add:
#ifdef USER_TEST
    int retUser = HMIAvAudioVideoAccessLibUser::getInstance().entIAV_GetAudioSettingsAvailability();
    if (retUser != RET_NOT_PROCESSED) {
        return (uc8_t)retUser;
    }
#endif // #ifdef USER_TEST
"""

"""
For bool_t entIAV_GetAasData(
                               uc8_t* pAasData,
                               uc8_t  buffSize,
                               uc8_t* pBytesWritten), please add:
#ifdef USER_TEST
    int retUser = HMIAvAudioVideoAccessLibUser::getInstance().entIAV_GetAasData(pAasData, buffSize, pBytesWritten);
    if (retUser != RET_NOT_PROCESSED) {
        return retUser == 0 ? FALSE : TRUE;
    }
