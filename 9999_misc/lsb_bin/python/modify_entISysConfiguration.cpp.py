...
ENTISYSCONFIGURATION_CPP_PATHNAME = """D:\casdev\WinCE\public\MMP_PROD\_SYS\Configuration\AccessLib\src\entISysConfiguration.cpp.orig""";
ENTISYSCONFIGURATION_CPP_PATHNAME_NEW = """D:\casdev\WinCE\public\MMP_PROD\_SYS\Configuration\AccessLib\src\entISysConfiguration.cpp""";
...
def IsExcludedFunc(strLine):
    for strExcludedFunc in EXCLUED_FUNCTIONS:
        strPatten = '^[ \t]*(void|BOOL|bool_t)[ \t]+%s\(.*$' % (strExcludedFunc);
...
def IsFunc(strLine):
    strPatten = r'^[ \t]*(void|BOOL|bool_t)[ \t]+(entISysConfiguration_|entISysVf_)[^(]+\(.*$';
    oRe = re.compile(strPatten);
    mo = oRe.match(strLine);
    if mo != None:
...
def ProcessFile(strFilePath):
    ...
    while True:
        ...
        if IsFunc(strLine):
            ...
            strOutputFileContent += "    uint32_t retUser = HMISysConfigurationAccessLibUser::getInstance().%s;%s" % (MakeFuncCallStr(oFuncNameAndArgs), NEW_LINE);
            ...
            strOutputFileContent += strLine;
            if strLine.strip() == "#include \"entISysConfiguration.h\"":
                strOutputFileContent += "#include \"entISysConfiguration_user.h\"%s" % (NEW_LINE);
...
def main():
    strNewFileContent = ProcessFile(ENTISYSCONFIGURATION_CPP_PATHNAME);
    oFile = file(ENTISYSCONFIGURATION_CPP_PATHNAME_NEW, "w+b");
    if oFile == None:
        LOG_ERR("*** ERROR: main(): can't open file \"%s\" to write!%s" % (ENTISYSCONFIGURATION_CPP_PATHNAME_NEW, NEW_LINE));
        exit(-1);
