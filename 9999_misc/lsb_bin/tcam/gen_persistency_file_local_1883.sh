...
function main()
{
    if [ -z "$NEW_PERSISTENCE_PACKAGE_PATH_NAME" ]; then
        NEW_PERSISTENCE_PACKAGE_PATH_NAME="tp-pers-install.tar.gz.local_1883"
    fi
    ...
    cat "$TMP_WORK_DIR/persistence_config_fsm_ssm.xml.orig" \
        | sed_in_scope.sh - "<name>signal_service_uri</name>" 10 "s/<default>.*<\/default>/<default>127.0.0.1:1883<\/default>/g" \
        | sed_in_scope.sh - "<name>signal_service_uri_ssl</name>" 10 "s/<default>.*<\/default>/<default>127.0.0.1:1883<\/default>/g" \
    ...
    cat "$TMP_WORK_DIR/persistence_config_remote.xml.orig" \
        | sed_in_scope.sh - "<name>message-delivery_MQTT_SERVER_URL</name>" 10 "s/<default>.*<\/default>/<default>127.0.0.1:1883<\/default>/g" \
