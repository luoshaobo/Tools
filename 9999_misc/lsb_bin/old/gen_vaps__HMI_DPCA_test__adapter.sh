...
LOG_FILE_PATH="d:/gen_vaps__HMI_DPCA_test__adapter.log"
...
if [ ! -z "$ALL_ADAPTORS" ]; then
    CMD_LINE=`cat <<EOF
cmd /c "C:\Presagis\VAPS_XT_32\bin\CodeGen.exe" "$ADAPTOR_PATH_PREFIX\HMI2\HMI_DPCA_test.vrpj" -codegen $ALL_ADAPTORS 2>&1 | tee $LOG_FILE_PATH
EOF`
    echo "$CMD_LINE"
    eval $CMD_LINE
fi