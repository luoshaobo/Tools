...
if [ "$ADK_INSTALLER_ROOT_DIR" = "default" ]; then
    HMI_IPK_FILE_PATH=`ls /PROJ/oip/GWM_MY18/MY18_SyDK/SyDK_${VERSION}_*/Mirror/solutions/com.continental/MV_SYSINT/prebuilt/arm-mv-linux/staging-sys-hmi-keyboard-armv7a-mv-linux_${VERSION}-r0_arm-mv-linux_*.ipk`
else
    HMI_IPK_FILE_PATH=`ls $ADK_INSTALLER_ROOT_DIR/Mirror/solutions/com.continental/MV_SYSINT/prebuilt/arm-mv-linux/staging-sys-hmi-keyboard-armv7a-mv-linux_${VERSION}-r0_arm-mv-linux_*.ipk`
fi
...
find `basename $WORK_TMP_DIR` -name "hmi-*"
echo "You can execute command line like: arm-none-linux-gnueabi-gdb -c core -e tmp/usr/bin/ovip/HMI/KeyboardService -s tmp/armv7a-mv-linux/usr/bin/ovip/HMI/.debug/KeyboardService"
